...
import MCQCounters from './MCQCounters'
...
const MCQ = ({game}: Props) => {
    ...
    const options = React.useMemo(()=>{
        ...
        return currentQuestions.options as string[];
    },[currentQuestions])
    ...
    <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 md:w-[80vw] max-w-4xl w-[90vw]'>
        ...
        <div className="flex flex-col items-center justify-center w-full mt-4">
            {options.map((option,index)=>{
                return(
                    <Button key={index} className='justify-start w-full py-8 mb-4' variant={selectedChoice === index? 'default': 'secondary'} onClick={()=>{
                        setSelectedChoice(index)
                    }}>
                        <div className="flex items-center justify-start">
                            <div className=" p-2 px-3  mr-5 rounded-md border">
                                {index + 1}
                            </div>
